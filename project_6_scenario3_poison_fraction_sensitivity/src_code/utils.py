...
def get_poison_fraction_level(poison_frac):
    """
    Scenario 3 helper.
    It actively classifies the poison fraction in the code, instead of only changing
    the command-line hyperparameter.
    """
    if poison_frac <= 0.05:
        return "very_low"
    elif poison_frac <= 0.10:
        return "low"
    elif poison_frac <= 0.25:
        return "medium"
    else:
        return "high"
...
def distribute_data(dataset, args, n_classes=10, class_per_agent=None):
    """
    Scenario 3 / Hypothesis 3 poison-fraction sensitivity data split.

    This scenario keeps the data distribution fixed and varies only poisoning intensity:
        poison_frac = 0.05, 0.10, 0.25, 0.50

    The code actively logs the scenario grid and the current poison fraction, so the
    txt files directly prove which fraction was used.
    """
    ...
    if hasattr(args, "verify_poison_fraction_sensitivity") and args.verify_poison_fraction_sensitivity == 1:
        print("VERIFY_POISON_FRACTION_SENSITIVITY_START")
        ...
        print(f"current_poison_fraction_level={get_poison_fraction_level(args.poison_frac)}")
        ...
        print("POISON_FRACTION_SENSITIVITY_PROOF: data split is held fixed; poison_frac is varied across the grid.")
        print("VERIFY_POISON_FRACTION_SENSITIVITY_PASSED")
        print("VERIFY_POISON_FRACTION_SENSITIVITY_END")
...
def poison_dataset(dataset, args, data_idxs=None, poison_all=False, agent_idx=-1,
    ...
    if len(all_idxs) == 0:
        ...
        print(f"poison_fraction_level={get_poison_fraction_level(poison_frac)}")
    ...
    actual_poison_ratio = num_poison / len(all_idxs) if len(all_idxs) > 0 else 0
    ...
    if hasattr(args, "verify_poisoning") and args.verify_poisoning == 1:
        ...
        print(f"actual_poison_ratio={actual_poison_ratio}")
        ...
        if clean_label_active:
            ...
            print("POISON_FRACTION_SENSITIVITY_POINT: clean-label effect is tested at this specific poison_frac.")
            ...
            print("POISON_FRACTION_SENSITIVITY_POINT: dirty-label effect is tested at this specific poison_frac.")
    ...
    return {
        ...
        "poison_fraction_level": get_poison_fraction_level(poison_frac),
        "actual_poison_ratio": actual_poison_ratio
    }
...
def print_exp_details(args):
    print('======================================')
    print('    Scenario: Hypothesis 3 - Poison fraction sensitivity')
    ...
    print(f'    Poison Frac Grid: {args.poison_frac_grid}')
    print(f'    Current Poison Frac: {args.poison_frac}')
    print(f'    Current Poison Fraction Level: {get_poison_fraction_level(args.poison_frac)}')
    ...
    if hasattr(args, "verify_poison_fraction_sensitivity"):
        print(f'    Verify Poison Fraction Sensitivity: {args.verify_poison_fraction_sensitivity}')
