...
def get_effective_pattern_type(args):
    """
    For clean-label I/II/III we make the experiments actually different.

    Clean-label I   -> plus trigger
    Clean-label II  -> square trigger
    Clean-label III -> apple trigger

    If clean_label_auto_pattern=0, then the script uses args.pattern_type exactly.
    """
...
def poison_dataset(dataset, args, data_idxs=None, poison_all=False, agent_idx=-1, force_dirty_label=False, context="train"):
    ...
    Evaluation/poisoned validation:
        We force dirty-label behavior with force_dirty_label=True.
        This tests whether base_class + trigger is classified as target_class.
    """
    ...
    if changed_pixels_first_sample is None:
        trigger_verification_passed = False
    else:
        trigger_verification_passed = changed_pixels_first_sample > 0
